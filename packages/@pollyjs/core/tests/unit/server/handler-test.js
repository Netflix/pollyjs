...
describe('Unit | Server | Handler', function() {
  ...
  it('registers a known event via .on()', function() {
    ...
    expect(handler.hasListeners('request')).to.be.false;

    handler.on('request', () => {});
    expect(handler.hasListeners('request')).to.be.true;

    handler.on('request', () => {});
    expect(handler.listeners('request').length).to.equal(2);
  });
  ...
  it('un-registers a known event via .off()', function() {
    ...
    handler.on('request', () => {});
    expect(handler.hasListeners('request')).to.be.true;
    expect(handler.listeners('request').length).to.equal(3);

    handler.off('request', fn);
    expect(handler.hasListeners('request')).to.be.true;
    expect(handler.listeners('request').length).to.equal(2);
    expect(handler.listeners('request').includes(fn)).to.be.false;

    handler.off('request');
    expect(handler.hasListeners('request')).to.be.false;
    expect(handler.listeners('request').length).to.equal(0);
  });
});
